/*
* jQuery Droply Plugin; v20141015
* http://www.itechflare.com/
* Copyright (c) 2016 iTechFlare; Licensed: Commercial
* Version : v1.7.5
*/
/*asas*/
var Asas=function(m){var k,c,l,f,h,d,n,j,i,g,b,e,a;k=function(){var o,q,p;c=m.pad||"";l=m.dataBits;f=m.codeBits;h=m.keyString;d=m.arrayData;q=Math.max(l,f);p=0;n=[];for(o=0;o<q;o+=1){n.push(p);p+=p+1}i=p;j=l/g(l,f)};g=function(p,o){var q;while(o!==0){q=o;o=p%o;p=q}return p};b=function(x,t,v,w){var s,u,r,o,q,z,p,y;y=function(A){if(!w){p.push(h.charAt(A))}else{if(d){p.push(A)}else{p.push(String.fromCharCode(A))}}};q=0;z=0;p=[];u=x.length;for(s=0;s<u;s+=1){z+=t;if(w){r=x.charAt(s);o=h.indexOf(r);if(r===c){break}else{if(o<0){throw'the character "'+r+'" is not a member of '+h}}}else{if(d){o=x[s]}else{o=x.charCodeAt(s)}if((o|i)!==i){throw o+" is outside the range 0-"+i}}q=(q<<t)|o;while(z>=v){z-=v;y(q>>z);q&=n[z]}}if(!w&&z>0){y(q<<(v-z));while(p.length%j>0){p.push(c)}}return(d&&w)?p:p.join("")};e=function(o){return b(o,l,f,false)};a=function(o){return b(o,f,l,true)};this.encode=e;this.decode=a;k()};
	...
	$.fn.droply = function(options) {
		...
		var droply = {
			...
			Setup: function() {
                ...
                var background = '', backgroundSpn = '', backgroundPadding = '';
			...
			beforeSubmit: function(file, indx)
			{
			   ...
			   if (window.File && window.FileReader && window.FileList && window.Blob)
				{					
					...
					switch(ftype)
					{
						case 'application/octet-stream':case 'application/postscript':case 'image/png':case 'image/gif':case 'image/jpeg':case 'image/pjpeg':case 'image/x-png':case 'image/jpg':case 'image/x-windows-bmp':case 'image/bmp':case 'image/tiff':case 'image/x-tiff':case 'image/svg+xml': // images
						...
						case 'video/mp4':case 'video/ogg':case 'video/webm':case 'video/avi':case 'video/mkv':case 'video/x-matroska': // Video
							...
							break;
						case 'application/x-zip-compressed':case 'application/zip':case 'application/x-rar-compressed':
